...
print('------------AMAZON CARD-------------\n')
    for _ in range(amount):
        part1 = ''.join(random.choices(strg.digits, k=4))
        part2 = ''.join(random.choices(strg.digits, k=6))
        part3 = ''.join(random.choices(strg.digits, k=4))
        amazon_code = f"{part1}-{part2}-{part3}"
        if verify(amazon_code):
            print(f"{Fore.GREEN} Valid | {Fore.RESET}{amazon_code}")
        else:
            print(f"{Fore.RED} Invalid | {Fore.RESET}{amazon_code}")
        time.sleep(1)
